...
export async function getLatestGitHubRelease(owner, repo, options = { filter: () => true }) {
    ...
    try {
        ...
        if (error instanceof SyntaxError) {
            throw new AbortError(`Received invalid response from GitHub API (HTTP ${fetchResult.status}).`, 'The response could not be parsed as JSON. The service may be temporarily unavailable. Please try again.');
        }
        throw error;
    }
}
